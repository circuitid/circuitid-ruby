  ...
  class Numbers
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircuitID::Numbers` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircuitID::Numbers`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [in_use, name, country_code, amount, per_minute_rate, fax, voice, status, sms, mms, directory_listing, caller_name, e911, message_campaign, message_class, message_type, e911_supported, caller_name_supported, directory_listing_supported, messaging_supported, destination_type, destination, ref, call_forwarding_destination].hash
    end
