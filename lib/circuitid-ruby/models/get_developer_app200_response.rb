...
module CircuitID
  class GetDeveloperApp200Response
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircuitID::GetDeveloperApp200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircuitID::GetDeveloperApp200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["events"])
      ...
      is_free_validator = EnumAttributeValidator.new('Integer', [1, 0])
      ...
      integration_type_validator = EnumAttributeValidator.new('String', ["webhook", "client"])
      ...
      status_validator = EnumAttributeValidator.new('String', ["active", "disabled", "suspended", "error", "pending"])
      return false unless status_validator.valid?(@status)
      webhook_auth_type_validator = EnumAttributeValidator.new('String', ["header", "usernameAndPassword"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["events"])
    ...
    def integration_type=(integration_type)
      validator = EnumAttributeValidator.new('String', ["webhook", "client"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["active", "disabled", "suspended", "error", "pending"])
    ...
    def webhook_auth_type=(webhook_auth_type)
      validator = EnumAttributeValidator.new('String', ["header", "usernameAndPassword"])
    ...
    def hash
      [name, description, type, visibility, require_id, is_free, fee_description, integration_type, services, events, user, webhook_url, terms_of_service_url, privacy_policy_url, permissions, status, webhook_auth_type, webhook_username, webhook_password, webhook_token_name, webhook_token, created_by_user_id, updated_by_user_id, created_at, updated_at].hash
    end
