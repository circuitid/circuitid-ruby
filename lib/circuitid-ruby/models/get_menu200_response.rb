  ...
  class GetMenu200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircuitID::GetMenu200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircuitID::GetMenu200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, max_extension_length, speech_recognition, directory, greeting_type, greeting_tts, menu_voice, exit_sound, transfer_announcement, max_failures, max_timeouts, timeout, touch_tone_terminators, timeschedule, destination_type, destination, ref, call_forwarding_destination, _id, created_by_user_id, updated_by_user_id, created_at, updated_at].hash
    end
