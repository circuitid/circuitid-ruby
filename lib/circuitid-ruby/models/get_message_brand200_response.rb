...
module CircuitID
  class GetMessageBrand200Response
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircuitID::GetMessageBrand200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircuitID::GetMessageBrand200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["verified", "unverified"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["verified", "unverified"])
    ...
    def hash
      [name, brand_id, ein, user, order, status, brand_relationship, vertical, entity_type, csp_id, ein_issuing_country, universal_ein, reference_id, created_by_user_id, updated_by_user_id, created_at, updated_at].hash
    end
