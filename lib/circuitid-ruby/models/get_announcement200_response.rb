...
module CircuitID
  class GetAnnouncement200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircuitID::GetAnnouncement200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircuitID::GetAnnouncement200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@text.nil? && @text.to_s.length < 2
        invalid_properties.push('invalid value for "text", the character length must be great than or equal to 2.')
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["tts", "mp3"])
      return false unless type_validator.valid?(@type)
      return false if !@text.nil? && @text.to_s.length > 255
      return false if !@text.nil? && @text.to_s.length < 2
      true
    end
    ...
    def text=(text)
      ...
      if !text.nil? && text.to_s.length < 2
        fail ArgumentError, 'invalid value for "text", the character length must be great than or equal to 2.'
      end
    ...
    def hash
      [attachments, name, type, voice, text, _id, created_by_user_id, updated_by_user_id, created_at, updated_at].hash
    end
