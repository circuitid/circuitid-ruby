# CircuitIDAPIClient::CustomersApi
...
> Object create_customer(customers)
...
api_instance = CircuitIDAPIClient::CustomersApi.new
customers = CircuitIDAPIClient::Customers.new({name: 3.56, default_bill_method: credit}) # Customers | The JSON object that will be posted to the REST API endpoint.

begin
  # Create a new object
  result = api_instance.create_customer(customers)
  p result
...
> <Array(Object, Integer, Hash)> create_customer_with_http_info(customers)

```ruby
begin
  # Create a new object
  data, status_code, headers = api_instance.create_customer_with_http_info(customers)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => Object
rescue CircuitIDAPIClient::ApiError => e
...
> <Array(Object, Integer, Hash)> get_customer_with_http_info(id)
...
> Object patch_customer(id, customers)
...
api_instance = CircuitIDAPIClient::CustomersApi.new
id = TODO # Object | The ObjectId (unique 12 bytes ID) of record you would like to GET.
customers = CircuitIDAPIClient::Customers.new({name: 3.56, default_bill_method: credit}) # Customers | The request data.

begin
  # Patch object's data
  result = api_instance.patch_customer(id, customers)
  p result
rescue CircuitIDAPIClient::ApiError => e
...
> <Array(Object, Integer, Hash)> patch_customer_with_http_info(id, customers)

```ruby
begin
  # Patch object's data
  data, status_code, headers = api_instance.patch_customer_with_http_info(id, customers)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => Object
rescue CircuitIDAPIClient::ApiError => e
...
> <Array(Object, Integer, Hash)> remove_customer_with_http_info(id)
